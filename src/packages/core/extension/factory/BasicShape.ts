...
// 创建矩形
export const createRect = (options?: RectProps) => new Rect({ ...DefaultProps.Rect, ...options })

// 创建圆形
export const createCircle = (options?: CircleProps) => new Circle({ ...DefaultProps.Circle, ...options })

// 创建三角形
export const createTriangle = (options?: FabricObjectProps) => new Triangle({ ...DefaultProps.Triangle, ...options })

// 创建多边形 - 待优化：做成自定义元素，传入顶点数+半径生成多边形（包括多边路径）
export const createPolygon = (options?: FabricObjectProps) => new Polygon([
    { x: 0, y: -50 },
    { x: 47, y: -15 },
    { x: 29, y: 40 },
    { x: -29, y: 40 },
...
// 创建椭圆
export const createEllipse = (options?: EllipseProps) => new Ellipse({ ...DefaultProps.Ellipse, ...options })
...
export const createStar = (options?: starProps) => {
