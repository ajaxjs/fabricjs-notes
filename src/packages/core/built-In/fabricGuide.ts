...
export class FabricGuide implements CorePluginTemp {
  ...
  private getCoords(obj: FabricObject): ACoords {
    const [tl, tr, br, bl] = obj.getCoords() as [Point, Point, Point, Point]
    return { tl, tr, br, bl }
  }
  ...
  private traversAllObjects(activeObject: FabricObject, canvasObjects: FabricObject[]) {
    ...
    for (let i = canvasObjects.length; i--;) {
      const canvasItem = canvasObjects[i]!
      const objCoords = {
        ...this.getCoords(canvasItem),
        c: canvasItem.getCenterPoint(),
      ...
      Keys(objCoordsByMovingDistance).forEach((activeObjPoint) => {
        const newCoords =
          canvasItem.angle !== 0 ? this.omitCoords(objCoords, 'horizontal') : objCoords
      ...
      Keys(objCoordsByMovingDistance).forEach((activeObjPoint) => {
        const newCoords =
          canvasItem.angle !== 0 ? this.omitCoords(objCoords, 'vertical') : objCoords
